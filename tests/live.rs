...
fn claude_answers_and_reports_usage() {
    ...
    nagoya::block_on(async {
        ...
        let outcome = run(&ping(Agent::Claude), &reactor.handle())
            .await
            .expect("claude run failed");
...
fn an_unknown_model_is_an_error_not_an_answer() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let err = run(&request, &reactor.handle())
            .await
...
#[test]
#[ignore = "spawns a real agent"]
fn codex_reports_an_unknown_model_as_a_sentence_not_json() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
fn the_codex_catalogue_still_matches_what_codex_reports() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let discovered = Agent::Codex
            .discover_models(&reactor.handle())
            .await
...
#[test]
fn agents_without_a_headless_listing_refuse_to_guess() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn effort_reaches_the_agents_that_take_it() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        for agent in [Agent::Claude, Agent::Codex] {
            ...
            let outcome = run(&ping(agent).effort("low"), &reactor.handle())
                .await
...
#[test]
#[ignore = "spawns a real agent"]
fn copilot_auto_still_refuses_an_effort() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let err = run(
            &ping(Agent::Copilot).model("auto").effort("low"),
            &reactor.handle(),
        )
        .await
...
#[test]
#[ignore = "spawns a real agent"]
fn a_bogus_effort_is_reported_not_ignored() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let err = run(&ping(Agent::Codex).effort("bogus-level"), &reactor.handle())
            .await
...
#[test]
#[ignore = "spawns a real agent"]
fn an_answer_about_limits_and_login_is_not_a_failed_run() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let request = Request::new(Agent::Claude, prompt)
            .permission(Permission::ReadOnly)
            .timeout(Duration::from_secs(180));
        let outcome = run(&request, &reactor.handle())
            .await
...
#[test]
fn codex_reports_account_usage_without_a_terminal() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
fn codex_interrupts_an_orphaned_active_turn() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        assert!(
            interrupt(&request, &reactor.handle())
                .await
...
fn codex_reads_a_long_idle_session_without_starting_a_turn() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn claude_reports_enough_to_track_context() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_1m_alias_reports_the_widened_window() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_token_parts_reconcile_with_the_total_it_reported() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let outcome = run(&ping(Agent::Codex), &reactor.handle())
            .await
            .expect("codex run failed");
        let usage = &outcome.usage;
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn copilot_reports_its_credit_spend() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let outcome = run(&ping(Agent::Copilot), &reactor.handle())
            .await
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_denied_approval_stops_the_tool_from_running() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let mut run = stream(&request, &reactor.handle()).expect("stream should start");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_message_sent_mid_turn_redirects_the_agent() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let dir =
            std::env::temp_dir().join(format!("agent-abstraction-duplex-{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("temp dir");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_app_server_accepts_a_live_steer() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let mut run = stream(&request, &reactor.handle()).expect("app-server should start");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_app_server_can_write_inside_its_cwd() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_app_server_routes_approvals() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_app_server_resumes_after_an_allowed_approval() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn live_usage_events_agree_with_the_final_outcome() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
fn a_follow_up_is_refused_where_it_cannot_be_delivered() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let plain = stream(&Request::new(Agent::Claude, PING), &reactor.handle()).expect("stream");
        assert!(
            matches!(
...
#[test]
fn approvals_are_refused_where_they_cannot_work() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        assert!(
            matches!(
                run(&discarded, &reactor.handle()).await,
                Err(agent_abstraction::Error::Unsupported { .. })
            ),
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_answers_and_reports_usage() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_runs_outside_a_git_repository() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_caller_assigned_session_id_is_honoured_and_resumable() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        for agent in [Agent::Claude, Agent::Copilot] {
            ...
            let first = run(&ping(agent).session_id(&chosen), &reactor.handle())
                .await
            ...
            let second = run(&ping(agent).resume(&chosen), &reactor.handle())
                .await
...
#[test]
fn codex_refuses_an_assigned_session_id() {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_reveals_its_thread_id_before_it_answers() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let mut running = stream(
            &ping(Agent::Codex).format(Format::Stream),
            &reactor.handle(),
        )
        .expect("spawn failed");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn every_agent_still_works_under_a_minimal_environment() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        for agent in Agent::ALL {
            ...
            let outcome = run(
                &ping(agent).env_policy(EnvPolicy::Minimal),
                &reactor.handle(),
            )
            .await
            .unwrap_or_else(|e| {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn codex_resumes_from_a_captured_thread_id_without_scraping() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let first = run(&first, &reactor.handle())
            .await
        ...
        let second = run(&second, &reactor.handle())
            .await
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn streaming_delivers_events_then_an_outcome() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let mut running = stream(&request, &reactor.handle()).expect("spawn failed");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_named_session_carries_context_across_turns() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn forking_branches_to_a_new_session() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let first = Request::new(Agent::Claude, "Remember the number 8813. Reply OK.")
            ...
            .session(&store, &project, name, false)
            .unwrap();
        let parent = run(&first, &reactor.handle())
            .await
            .expect("first turn failed");
        ...
        let forked = run(&forked, &reactor.handle())
            .await
            .expect("forked turn failed");
...
#[test]
fn a_missing_agent_reports_how_to_install_it() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        let request = Request::new(Agent::Codex, "hi").bin("agent-abstraction-no-such-binary");
        let err = run(&request, &reactor.handle()).await.unwrap_err();
...
#[test]
fn installed_agents_match_the_versions_the_flags_were_verified_against() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        for agent in Agent::ALL {
            ...
            let probe = Probe::run(agent, &reactor.handle())
                .await
                .expect("probe failed");
            checked += 1;
...
#[test]
fn installed_agents_report_their_login_state() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        for agent in Agent::ALL {
            ...
            let status = AuthStatus::check(agent, &reactor.handle())
                .await
                .expect("check failed");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_schema_constrains_the_answer_to_data() {
    const SCHEMA: &str = r#"{
        "type": "object",
    ...
    nagoya::block_on(async {
        for agent in [Agent::Claude, Agent::Codex] {
            ...
            let value = outcome.structured.unwrap_or_else(|| {
                panic!("{agent} returned no structured answer: {:?}", outcome.text)
            });
...
#[test]
fn copilot_refuses_a_schema_before_spawning() {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn a_codex_schema_file_is_cleaned_up() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let outcome = run(
            &Request::new(Agent::Codex, "Alice is 30 years old.")
                .permission(Permission::ReadOnly)
                .timeout(Duration::from_secs(180))
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn text_arrives_in_pieces_before_the_run_finishes() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn claude_runs_compact_as_a_command_and_reports_its_phases() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        ...
        let mut stream = stream(&request, &reactor.handle()).expect("compact run failed to start");
...
#[test]
#[ignore = "spawns a real agent and consumes quota"]
fn only_claude_takes_a_slash_command() {
    // Each test owns its reactor and keeps it alive to the end: a dropped
    // reactor stops the thread that wakes the child's pipes.
    let reactor = nagoya::reactor::Reactor::start().expect("reactor");
    nagoya::block_on(async {
        for agent in [Agent::Codex, Agent::Copilot] {
            ...
            let error = run(&request, &reactor.handle())
                .await
